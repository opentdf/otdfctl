...
import (
	...
	"github.com/opentdf/otdfctl/tui/crud"
...
func InitAttributeView(id string, h handlers.Handler) (AttributeView, tea.Cmd) {
	...
	model, _ := crud.InitRead("Read Attribute", items)
	m := AttributeView{sdk: h, attr: attr, read: model.(crud.Read)}
	model, msg := m.Update(common.WindowMsg())
