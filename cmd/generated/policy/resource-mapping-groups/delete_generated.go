...
type DeleteRequestFlags struct {
	Id string `json:"id"` // The ID of the resource mapping group to delete
	Force string `json:"force"` // Force deletion without interactive confirmation (dangerous)
}
...
func NewDeleteCommand(handler DeleteHandler) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "delete",
		Short:   "Delete a resource mapping group",
	...
	// Register flags
	cmd.Flags().String("id", "", "The ID of the resource mapping group to delete")
	cmd.Flags().String("force", "", "Force deletion without interactive confirmation (dangerous)")
