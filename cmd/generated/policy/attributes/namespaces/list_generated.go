...
type ListRequestFlags struct {
	State string `json:"state"` // Filter by state [active, inactive, any]
...
func NewListCommand(handler ListHandler) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "list",
		Aliases: []string{"ls", "l"},
		Short:   "List attribute namespaces",
	...
	// Register flags
	cmd.Flags().StringP("state", "s", "", "Filter by state [active, inactive, any]")
