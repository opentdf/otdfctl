...
// RotateRequestFlags represents the flags for the rotate command
type RotateRequestFlags struct {
	Key string `json:"key"` // The KeyID (human-readable identifier) or the internal UUID of the existing key to rotate from the specified KAS. The system will attempt to resolve the provided value as either a UUID or a KeyID.
	Kas string `json:"kas"` // Specify the Key Access Server (KAS) where the key is registered. The KAS can be identified by its ID, URI, or Name.
	...
	WrappingKey string `json:"wrappingKey"` // The symmetric key material (AES cipher, base64 encoded) used to wrap the generated private key. Primarily used when `mode` is `local`.
...
// RotateRequest represents the parameters for the rotate command
type RotateRequest struct {
	Flags RotateRequestFlags `json:"flags"`
	RawArguments []string `json:"raw_arguments"` // Raw command line arguments passed to the command
}

// RotateHandler defines the function type for handling rotate commands
type RotateHandler func(cmd *cobra.Command, req *RotateRequest) error

// NewRotateCommand creates a new rotate command with the provided handler function
func NewRotateCommand(handler RotateHandler) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "rotate",
		Aliases: []string{"r"},
		Short:   "Rotate Key",
		RunE: func(cmd *cobra.Command, args []string) error {
			return runRotate(cmd, args, handler)
		},
	}
	...
	// Register flags
	cmd.Flags().StringP("key", "k", "", "The KeyID (human-readable identifier) or the internal UUID of the existing key to rotate from the specified KAS. The system will attempt to resolve the provided value as either a UUID or a KeyID.")
	cmd.MarkFlagRequired("key")
	cmd.Flags().String("kas", "", "Specify the Key Access Server (KAS) where the key is registered. The KAS can be identified by its ID, URI, or Name.")
	...
	cmd.Flags().StringP("wrapping-key", "w", "", "The symmetric key material (AES cipher, base64 encoded) used to wrap the generated private key. Primarily used when `mode` is `local`.")
...
// runRotate handles argument and flag extraction
func runRotate(cmd *cobra.Command, args []string, handler RotateHandler) error {
