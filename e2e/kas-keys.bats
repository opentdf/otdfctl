...
@test "kas-keys: create key (local mode, rsa:2048)" {
  ...
  run_otdfctl_key create --kas "${KAS_REGISTRY_ID}" --key-id "${KEY_ID}" --algorithmorithm rsa:2048 --mode local --wrapping-key-id wrapping-key-1 --wrappingKey "${WRAPPING_KEY}" --json
  assert_success
...
@test "kas-keys: create key (local mode, ec:secp256r1)" {
  ...
  # Its exact value cannot be known before running the command without replicating the key generation logic.
  # Thus, we assert its presence and that it\'s a non-empty base64 encoded string. This is the intended assertion.
  run_otdfctl_key create --kas "${KAS_REGISTRY_ID}" --key-id "${KEY_ID}" --algorithm "ec:secp256r1" --mode "local" --wrapping-key-id "wrapping-key-1" --wrappingKey "${WRAPPING_KEY}" --json
  assert_success
  assert_equal "$(echo "$output" | jq -r .kas_id)" "${KAS_REGISTRY_ID}"
  assert_equal "$(echo "$output" | jq -r .key.key_id)" "${KEY_ID}"
...
@test "kas-keys: create key (missing key-id)" {
  run_otdfctl_key create --kas "${KAS_REGISTRY_ID}" --algorithm "rsa:2048" --mode "local" --wrapping-key-id "wrapping-key-1" --wrappingKey "${WRAPPING_KEY}"
  assert_failure
...
@test "kas-keys: create key (missing algorithm)" {
  KEY_ID=$(generate_key_id)
  run_otdfctl_key create --kas "${KAS_REGISTRY_ID}" --key-id "${KEY_ID}" --mode "local" --wrapping-key-id "wrapping-key-1" --wrappingKey "${WRAPPING_KEY}"
  assert_failure
...
@test "kas-keys: create key (missing mode)" {
  KEY_ID=$(generate_key_id)
  run_otdfctl_key create --kas "${KAS_REGISTRY_ID}" --key-id "${KEY_ID}" --algorithm "rsa:2048" --wrapping-key-id "wrapping-key-1" --wrappingKey "${WRAPPING_KEY}"
  assert_failure
...
@test "kas-keys: create key (local mode, missing wrapping-key-id)" {
  KEY_ID=$(generate_key_id)
  run_otdfctl_key create --kas "${KAS_REGISTRY_ID}" --key-id "${KEY_ID}" --algorithm "rsa:2048" --mode "local" --wrappingKey "${WRAPPING_KEY}"
  assert_failure
...
@test "kas-keys: create key (missing kas identifier - kas, kas, or kas)" {
  KEY_ID=$(generate_key_id)
...
@test "kas-keys: get key (not found by user key-id and kas)" {
  run_otdfctl_key get --key-id "non-existent-key" --kas "${KAS_REGISTRY_ID}" --json
  assert_failure
  assert_output --partial "Failed to get kas key" # Error should indicate not found or similar
}
...
@test "kas-keys: get key (mutually exclusive: kas and kas with key-id)" {
  run_otdfctl_key get --key-id "some-user-key-id" --kas "${KAS_REGISTRY_ID}" --kas "some-kas-name" --json
  assert_failure
  assert_output --partial "Error: if any flags in the group [kas kas kas] are set none of the others can be; [kas kas] were all set"
}
...
@test "kas-keys: list keys (mutually exclusive: kas and kas)" {
  run_otdfctl_key list --kas "${KAS_REGISTRY_ID}" --kas "some-kas-name" --json
  assert_failure
  assert_output --partial "Error: if any flags in the group [kas kas kas] are set none of the others can be"
}
